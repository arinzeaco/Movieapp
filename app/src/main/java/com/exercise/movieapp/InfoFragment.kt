...
class InfoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fragmentInfoBinding.rating.text = movies.ratings


        Glide.with(fragmentInfoBinding.posterImg).
        load(movies.poster)
            .into(fragmentInfoBinding.posterImg)


        fragmentInfoBinding.fabSave.setOnClickListener {
            val datas = MoviesFavorite(
                movies.id, movies.idd, movies.title, movies.poster,
                movies.ratings, movies.description
            )
            viewModel.saveFavoriteMovies(datas)
            Snackbar.make(view, "Saved Successfully!", Snackbar.LENGTH_LONG).show()
        }
        fragmentInfoBinding.fabHide.setOnClickListener {
            val datas = MoviesHide(
                movies.id, movies.idd, movies.title, movies.poster,
                movies.ratings, movies.description
            )
